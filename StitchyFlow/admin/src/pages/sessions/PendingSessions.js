/**
 * Pending Sessions - Live Database Page
...
import { Search, Refresh, HourglassEmpty } from '@mui/icons-material';
...
export default function PendingSessions() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/sessions/pending${qs}`, {
  ...
  return (
    <Layout title="Pending Sessions">
      <Box>
        <Box sx={{ display:'flex', alignItems:'center', justifyContent:'space-between', mb:3 }}>
          <Box sx={{ display:'flex', alignItems:'center', gap:2 }}>
            <Box sx={{ width:48, height:48, borderRadius:'12px', bgcolor:'#ede9fe', display:'flex', alignItems:'center', justifyContent:'center' }}>
              <HourglassEmpty sx={{ color:'#7c3aed', fontSize:26 }} />
            </Box>
            <Box>
              <Typography variant="h6" sx={{ fontWeight:700, color:'#0f172a' }}>Pending Sessions</Typography>
              <Typography variant="caption" sx={{ color:'#64748b' }}>Sessions awaiting verification or approval — live from database</Typography>
            </Box>
          </Box>
          <Box sx={{ display:'flex', alignItems:'center', gap:1 }}>
            <Chip label={`${rows.length} records`} size="small" sx={{ bgcolor:'#ede9fe', color:'#7c3aed', fontWeight:700 }} />
                ...
                <Table size="small" stickyHeader>
                  <TableHead>
                    <TableRow sx={{ '& th': { bgcolor:'#f8fafc', fontWeight:700, fontSize:'0.72rem', color:'#475569', textTransform:'uppercase', letterSpacing:'0.04em', borderBottom:'2px solid #e2e8f0' } }}>
                      ...
                      <TableCell>Last Active</TableCell><TableCell>Expires At</TableCell><TableCell>Created At</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {paginated.length === 0 ? (
                      <TableRow><TableCell colSpan={11} align="center" sx={{ py:6, color:'#94a3b8' }}>No pending sessions found</TableCell></TableRow>
                    ) : paginated.map((row, i) => (
                      <TableRow key={row.session_id} hover sx={{ '&:hover': { bgcolor:'#f5f3ff' } }}>
                        <TableCell sx={{ color:'#94a3b8', fontSize:'0.78rem', fontWeight:600 }}>{page * rpp + i + 1}</TableCell>
                        <TableCell>
                          <Box sx={{ display:'flex', alignItems:'center', gap:1.5 }}>
                            <Avatar sx={{ width:32, height:32, fontSize:'0.75rem', fontWeight:700, bgcolor:'#7c3aed' }}>{(row.user_name || 'U')[0].toUpperCase()}</Avatar>
                            <Box>
                        ...
                        <TableCell><Box sx={{ bgcolor:'#ede9fe', border:'1px solid #ddd6fe', borderRadius:'6px', px:1, py:0.3, display:'inline-block' }}><Typography sx={{ fontSize:'0.75rem', fontFamily:'monospace', color:'#7c3aed', fontWeight:600 }}>{row.ip_address || '—'}</Typography></Box></TableCell>
                        ...
                        <TableCell><Chip label="pending" size="small" sx={{ bgcolor:'#ede9fe', color:'#6d28d9', fontWeight:700, fontSize:'0.68rem', height:22 }} /></TableCell>
                        ...
                        <TableCell><Typography sx={{ fontSize:'0.78rem', color:'#334155' }}>{row.expires_at ? new Date(row.expires_at).toLocaleString('en-PK') : '—'}</Typography></TableCell>
